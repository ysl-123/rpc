...
{ // Descriptor这是全局存放的
    // 每一个内置方法里面调用callmethod的时候填入的method都是不一样的
    ...
    std::string args_str;
    if (request->SerializeToString(&args_str))//就是request序列化给args_str
    {
    ...
    std::cout << "rpc_header_str: " << rpc_header_str << std::endl;//实际上有问题，都已经序列化了
